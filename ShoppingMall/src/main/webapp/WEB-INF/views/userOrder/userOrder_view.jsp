...
<%--                     </c:forEach> --%>
					...
					<c:set var="totalPriceNum" value="${totalPrice}" />
					<c:set var="userOrderTotalPriceNum" value="${userOrder.totalprice}" />
					
					<c:set var="totalPriceNum" value="${totalPriceNum.trim()}" />
					<c:set var="userOrderTotalPriceNum" value="${userOrderTotalPriceNum.trim()}" />
					
					<c:if test="${not empty totalPriceNum and not empty userOrderTotalPriceNum}">
					  <c:set var="totalPrice" value="${totalPriceNum + userOrderTotalPriceNum}" />
					</c:if>
                    <tr height="50">
                      <td> <strong> 총 주문금액 </strong> </td>
                      <td align="right">
                        <strong>
                          <fmt:formatNumber value="${totalPriceNum + userOrderTotalPriceNum}" pattern="##,###,###" />원
                        </strong>
                      </td>
                    </tr>
                  </table>
