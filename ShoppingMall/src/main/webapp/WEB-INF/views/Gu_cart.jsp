    ...
    -->
			...
			<aside id="aisdeLeft">
				...
				<div class="notice" style="position: absolute; top: 400px;">
					<ul>
						<li><a href="/ShoppingMall/notice/list">Notice</a></li>
						<li><a href="/ShoppingMall/Qna_list">QnA</a></li>
						<li><a href="/ShoppingMall/rev_list">Review</a></li>
					</ul>
				</div>
			...
			<section id="section" style="width: 600px;">
				<article>
					<div id="article" style="height: auto;">
						...
						<div>
							<div style="margin-top: 30px; font-size: small;">장바구니</div>
							<table style="width: 100%; margin-top: 60px;" align="center">
								<tr style="text-align: center;">
									<td style="width: 30px; height: 5px; padding: 0px;"><input
										type="checkbox" id="checkAll" checked="checked"
								...
								<c:forEach var="cart" items="${cartList}" varStatus="status">
									<tr id="productRow${status.index + 1}">
										<td>
											<!-- input checkbox --> <input type="hidden"
											value="${cart.cartcode}"> <input type="checkbox"
											id="check_${status.index + 1}" class="chBox"
											data-cartNum="${cart.cartcode}" checked="checked"> <%-- <input type="checkbox" id="check_${status.index + 1}" class="chBox"> --%>
										</td>
										<td>${cart.prothumb}</td>
										<!-- image -->
                              ...
                              / <span id="psize${status.index + 1}">${cart.psize}</span>
										...
										<td>
											<!-- 금액 --> <span style="float: left;">&#8361;</span>
											<div id="proprice${status.index + 1}">${cart.proprice}</div>
											<span style="float: left;">&#8361;</span>
											<div id="prosale${status.index + 1}">${cart.prosale}</div>
										</td>
										<td>
											<!-- Qty --> <input id="quantityN${status.index + 1}"
											type="text" value="${cart.amount}"
											data-cartNum="${cart.cartcode}" min="1" size="1"
											maxlength="2"> <a href="#" id="quantityUp"
											data-cartNum="${status.index + 1}"><img
												src="resources/plus.png" alt="up" width="10px" height="10px"></a>
											<a href="#" id="quantityDown"
											data-cartNum="${status.index + 1}"><img
												src="resources/minus.png" alt="down" width="10px"
												height="10px"></a>
										</td>
										<td>기본배송</td>
										<td>[조건]</td>
										<!-- 무료배송 -->
										<td>
											<!-- total 수량*금액 --> <span style="float: left;">&#8361;</span>
											<span id="total${status.index + 1}"></span>
										</td>
										<td>
											<!-- choice --> <input id="order${status.index + 1}"
											type="button" value="주문하기"><br> <input
											id="removeBtn_${status.index + 1}"
											data-cartNum="${cart.cartcode}" type="button" value="삭제하기">
										</td>
									</tr>
								</c:forEach>
						...
						<div
							style="font-size: small; color: gray; float: left; margin-top: 10px;">
							할인 적용 금액은 주문작성의 결정예정금액에서 확인 가능합니다.</div>
						<div>
							<hr style="width: 100%;">
						</div>
						<div style="float: left; font-size: small; margin-bottom: 40px;">
							선택상품을 <input type="button" id="selectRemove" value="삭제하기"
...
<script type="text/javascript">
	...
	$(document).ready(
			function() {
				$("tr[id^=productRow]").each(
						function() {
							var row = $(this);
							var quantity = parseInt(row.find(
									"input[id^=quantityN]").val());
							var price = parseInt(row.find("div[id^=prosale]")
									.text().replace(/[^0-9]/g, ''));
							if (!isNaN(quantity) && !isNaN(price)) {
								var total = quantity * price;
								row.find("span[id^=total]").text(
		...
		$("tr[id^=productRow]").each(
				function() {
					...
					if (checkbox.prop("checked")) {
						var subTotal = parseInt(row.find("span[id^=total]")
								.text().replace(/[^0-9]/g, ''));
						if (!isNaN(subTotal)) {
							total += subTotal;
